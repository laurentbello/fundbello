#!/usr/bin/env python3
"""Send a pre-written watchlist digest as email via Gmail SMTP. No LLM/API calls — just delivery.

The Claude Code scheduled session does the research/monitoring on your membership, writes the
digest JSON, then runs this to send it:

    python3 watchlist/send_email.py path/to/digest.json
